import { getTotalAmountDue, getAllStudents } from "../reminder";
import type {
  EligibleParent,
...
const makePeriod = (
  overrides: Partial<EligibleParentPeriod> = {},
): EligibleParentPeriod => ({
  school_month: "June",
  year: 2026,
  was_sent: false,
...
const makeParent = (
  overrides: Partial<EligibleParent> = {},
): EligibleParent => ({
  id: 1,
...
describe("getTotalAmountDue", () => {
  it("sums total_amount across all unpaid periods", () => {
    const parent = makeParent({
      unpaid_periods: [
        makePeriod({ total_amount: 810 }),
        makePeriod({ total_amount: 900 }),
      ],
    });
  ...
  it("returns the single period amount when only one period", () => {
    const parent = makeParent({
      unpaid_periods: [makePeriod({ total_amount: 1620 })],
    });
