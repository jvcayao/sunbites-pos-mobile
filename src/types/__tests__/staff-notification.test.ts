...
describe("getNotificationTitle", () => {
  ...
  it("returns pre-registration title with student name", () => {
    expect(getNotificationTitle(preRegistration)).toBe(
      "New Pre-Registration: Maria Santos",
    );
  });
});
...
describe("getNotificationRoute", () => {
  ...
  it("returns pre-registration route with id", () => {
    expect(getNotificationRoute(preRegistration)).toBe(
      "/(app)/pre-registrations/7",
    );
  });
});
