...
export const studentsApi = {
  ...
  enroll: (data: object) => client.post("/enrollment", data),

  // List & detail
  list: (params?: object) => client.get("/students", { params }),
  show: (id: number) => client.get(`/students/${id}`),
  update: (id: number, data: object) => client.put(`/students/${id}`, data),
  destroy: (id: number) => client.delete(`/students/${id}`),

  // Status & type
  updateStatus: (
    id: number,
    data: { enrollment_status: string; reason?: string },
  ...
  // Credit
  settleCredit: (id: number) => client.post(`/students/${id}/credit/settle`),
