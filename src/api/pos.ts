...
export const posApi = {
  ...
  getStudent: (id: number) => client.get(`/pos/students/${id}`),

  // Menu items
  menuItems: () => client.get<PosMenuItem[]>("/pos/menu-items"),
  createMenuItem: (data: {
    name: string;
    price: number;
    category: string;
  ...
  toggleMenuItem: (id: number) => client.post(`/pos/menu-items/${id}/toggle`),
  deleteMenuItem: (id: number) => client.delete(`/pos/menu-items/${id}`),

  // Linked stock (ingredient mapping)
  getLinkedStock: (menuItemId: number) =>
    client.get<LinkedStockItem[]>(
  ...
  // POS inventory
  posInventory: () => client.get("/pos/inventory"),
