...
export function CashInput({ total, tendered, onTenderedChange }: CashInputProps) {
    ...
    <View style={styles.container}>
      ...
      {tendered !== '' && !isInsufficient && (
        <View style={styles.changeRow}>
          <Text variant="bodyMedium" style={styles.changeLabel}>Change:</Text>
      ...
      {tendered !== '' && isInsufficient && (
        <Text variant="bodySmall" style={styles.insufficientText}>
          Insufficient — need {formatCurrency(total - parseFloat(tendered))} more
        </Text>
      )}
    </View>
  )
