...
export function CartPanel({ onCheckout, loading = false }: CartPanelProps) {
  ...
  const canCheckout = useMemo(() => {
    ...
    return true
  }, [items, paymentMethod, cashTendered, total, student, computedDiscount, discountReason])

  const handleConfirm = (): void => {
    const payload: CheckoutPayload = {
      student_id: student?.id,
      payment_method: paymentMethod as OrderPaymentMethod,
      items: items.map((i) => ({ pos_menu_item_id: i.menuItem.id, quantity: i.quantity })),
      ...
      amount_tendered: paymentMethod === 'cash' ? parseFloat(cashTendered) : undefined,
      reference_number: paymentMethod === 'gcash' ? gcashRef || undefined : undefined,
    }
    onCheckout(payload)
  ...
  return (
    ...
    >
      <Text variant="titleMedium" style={styles.heading}>Order Summary</Text>
      <Divider />
      ...
      >
        {items.length === 0 ? (
          <EmptyState icon="cart-outline" title="Cart is empty" subtitle="Tap a menu item to add it" />
        ) : (
      ...
      <CheckoutConfirmSheet
        ...
        amountTendered={paymentMethod === 'cash' ? parseFloat(cashTendered) : undefined}
        loading={loading}
...
const styles = StyleSheet.create({
  ...
  scroll:   { flex: 1 },
  itemList: { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: palette.zinc200 },
