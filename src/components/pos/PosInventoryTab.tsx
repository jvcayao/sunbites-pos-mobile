...
export function PosInventoryTab() {
  ...
  const [adjustTarget, setAdjustTarget] = useState<PosInventoryItem | null>(
    null,
  );

  const items: PosInventoryItem[] = Array.isArray(data)
    ? data
    : ((data as any)?.data ?? []);
    ...
    ({ item }: { item: PosInventoryItem }) => (
      <View style={[listCardStyle, styles.row]}>
        ...
        >
          ...
          />
          <Text variant="labelSmall" style={styles.adjustLabel}>
            Adjust
          </Text>
        </Pressable>
      </View>
  ...
  const keyExtractor = useCallback(
    (item: PosInventoryItem) => String(item.id),
    [],
  );
