...
function AddMenuItemForm({
  onCreate,
  isCreating,
}: AddMenuItemFormProps): React.JSX.Element {
  const {
    control,
    handleSubmit,
    reset,
  ...
  return (
    <View style={formStyles.container}>
      <Text variant="labelMedium" style={formStyles.heading}>
        + Add New Item
      </Text>
      <Controller
        control={control}
      ...
      <Controller
        ...
        render={({ field: { value, onChange } }) => (
          <View>
            <Text variant="labelSmall" style={formStyles.fieldLabel}>
              Category *
            </Text>
            <FilterChipRow>
              {CATEGORIES.map((c) => (
...
export function MenuMgmtTab(): React.JSX.Element {
  ...
  const handleToggle = useCallback((item: PosMenuItem): void => {
    toggleItem(item.id, {
      onSuccess: () =>
      ...
      <View style={[listCardStyle, styles.row]}>
          ...
          <View style={styles.metaRow}>
            ...
            {!item.has_inventory_mapping && (
              <View style={styles.notLinkedBadge}>
                <Text variant="labelSmall" style={styles.notLinkedText}>
                  Not linked
                </Text>
              </View>
            )}
        ...
        <View style={styles.actions}>
          ...
          <Button
            compact
            mode="outlined"
            onPress={() => setLinkTarget(item)}
            accessibilityRole="button"
            accessibilityLabel={`Link stock for ${item.name}`}
          >
            Link Stock
          </Button>
          <Button
  ...
  }, [handleToggle, setLinkTarget, setDeleteTarget]);
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListHeaderComponent={
          <AddMenuItemForm onCreate={handleCreate} isCreating={isCreating} />
        }
        refreshControl={
          <RefreshControl
        ...
        ListEmptyComponent={
          <EmptyState
            icon="food"
            title="No menu items"
            subtitle="Add the first item above"
          />
        }
