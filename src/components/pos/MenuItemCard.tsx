...
function getItemState(item: PosMenuItem): {
  ...
  if (item.is_subscription_item === null)
    return { isDisabled: true, badge: "none" };
  if (item.inventory_status === "OUT")
    return { isDisabled: true, badge: "out" };
  if (!item.has_inventory_mapping)
    return { isDisabled: true, badge: "not_linked" };
  if (item.inventory_status === "LOW")
    return { isDisabled: false, badge: "low" };
  if (item.is_subscription_item === true)
    return { isDisabled: false, badge: "subscription" };
  return { isDisabled: false, badge: "none" };
}

export const MenuItemCard = function MenuItemCard({
  item,
  cartQuantity,
  onPress,
      ...
      >
        <View style={[styles.card, cartQuantity > 0 && styles.cardInCart]}>
          ...
          <View style={styles.awning}>
            <Text style={styles.awningName} numberOfLines={2}>
              {item.name}
            </Text>
          </View>
          <View style={styles.body}>
            ...
            {statusBadge === "out" && (
              <View style={[styles.badge, styles.badgeOut]}>
                <Text style={[styles.badgeText, styles.badgeOutText]}>
                  Out of Stock
                </Text>
              </View>
            )}
            {statusBadge === "not_linked" && (
              <View style={[styles.badge, styles.badgeNotLinked]}>
                <Text style={[styles.badgeText, styles.badgeNotLinkedText]}>
                  Not linked
                </Text>
              </View>
            )}
            {statusBadge === "low" && (
              <View style={[styles.badge, styles.badgeLow]}>
                <Text style={[styles.badgeText, styles.badgeLowText]}>
                  Low Stock
                </Text>
              </View>
            )}
            {statusBadge === "subscription" && (
              <View style={[styles.badge, styles.badgeSub]}>
                <Text style={[styles.badgeText, styles.badgeSubText]}>
                  Subscription
                </Text>
              </View>
            )}
