...
import { useLinkedStock, useAttachLinkedStock, useDetachLinkedStock, usePosInventory } from '@/hooks/usePos'
...
export function LinkedStockSheet({ menuItemId, menuItemName, visible, onClose }: LinkedStockSheetProps) {
  const toast = useToast()
  const [selectedInventoryId, setSelectedInventoryId] = useState<number | null>(null)
  const [qtyInput, setQtyInput]                       = useState('1')
  const [removeTarget, setRemoveTarget]               = useState<LinkedStockItem | null>(null)

  const { data: rawLinked, isLoading: isLoadingLinked } = useLinkedStock(menuItemId)
  ...
  const handleAttach = (): void => {
    ...
    if (qty < 1) return
    attach(
      { menuItemId, data: { inventory_item_id: selectedInventoryId, quantity_used: qty } },
      {
        onSuccess: () => { toast.success('Stock linked'); setSelectedInventoryId(null); setQtyInput('1') },
  ...
  const handleDetach = (): void => {
    ...
    detach(
      ...
      {
        onSuccess: () => { toast.success('Link removed'); setRemoveTarget(null) },
        onError: (err: unknown) => toast.error(getApiError(err)),
      },
    )
  }
      ...
      <Modal visible={visible} onDismiss={onClose} contentContainerStyle={styles.modal}>
        <Surface style={styles.surface} elevation={4}>
          ...
          <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
            ...
            {isLoadingLinked ? (
              ...
              <>
                <View style={styles.tableHeader}>
                  <Text variant="labelSmall" style={[styles.col1, styles.headerText]}>Inventory Item</Text>
                  <Text variant="labelSmall" style={[styles.col2, styles.headerText]}>Qty/Sale</Text>
                  <Text variant="labelSmall" style={[styles.col3, styles.headerText]}>Action</Text>
                </View>
                {linked.map((item) => (
                  <View key={item.id} style={styles.tableRow}>
                    <Text variant="bodySmall" style={styles.col1}>{item.name}</Text>
                    <Text variant="bodySmall" style={styles.col2}>{item.quantity_used} {item.unit}</Text>
                    <Button
                      compact
                      mode="text"
            ...
            {/* Add Link form */}
            <Text variant="labelMedium" style={styles.addTitle}>+ Add Link</Text>
            <View style={styles.addRow}>
              {/* Inventory item selector — simple list since Picker needs native dependency */}
              <View style={styles.selectorWrap}>
                <Text variant="labelSmall" style={styles.meta}>Select Inventory Item</Text>
                {available.length === 0 ? (
                  <Text variant="bodySmall" style={styles.meta}>All items already linked</Text>
                ) : (
                  available.map((inv) => (
                    <Button
                      key={inv.id}
                      compact
                      mode={selectedInventoryId === inv.id ? 'contained' : 'outlined'}
              ...
              <View style={styles.qtyRow}>
                ...
                <Button
                  ...
                  loading={isAttaching}
                  disabled={isAttaching || selectedInventoryId === null || parseInt(qtyInput, 10) < 1}
                  accessibilityRole="button"
                >
                  Add Link
                </Button>
          ...
          <View style={styles.footer}>
            <Button mode="outlined" onPress={onClose} accessibilityRole="button">Close</Button>
          </View>
        </Surface>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  tableHeader:  { flexDirection: 'row', paddingVertical: 6, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: palette.zinc200 },
  tableRow:     { flexDirection: 'row', alignItems: 'center', paddingVertical: 6, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: palette.zinc200 },
  ...
  warning:      { marginTop: 12, padding: 12, backgroundColor: '#FFF7ED', borderRadius: 8 },
  warningText:  { color: '#92400E' },
  footer:       { padding: 16, flexDirection: 'row', justifyContent: 'flex-end', borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: palette.zinc200 },
})
