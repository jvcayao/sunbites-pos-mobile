...
export function PrintQrSheet({ visible, studentName, qrCode, onDismiss }: PrintQrSheetProps) {
    ...
    <Portal>
      <Modal visible={visible} onDismiss={onDismiss} contentContainerStyle={styles.modal}>
        <Surface style={styles.surface} elevation={4}>
          <Text variant="titleMedium" style={styles.heading}>QR Code — {studentName}</Text>
          <Divider />
          <View style={styles.body}>
            <View style={styles.qrContainer}>
              <QrCodeSvg value={qrCode} frameSize={160} />
              <Text variant="labelSmall" style={styles.qrLabel}>{qrCode}</Text>
            </View>
            <Text variant="labelSmall" style={styles.perRowLabel}>Cards per row</Text>
            <FilterChipRow>
              {[2, 4].map((n) => (
                <FilterChip key={n} label={String(n)} active={cardsPerRow === n} onPress={() => setCardsPerRow(n)} />
              ))}
            </FilterChipRow>
          </View>
          <View style={styles.actions}>
            <Button onPress={onDismiss} accessibilityRole="button">Close</Button>
            <Button
              mode="contained"
              icon="share-variant"
              onPress={handleShare}
...
const styles = StyleSheet.create({
  ...
  perRowLabel: { color: palette.zinc500, textTransform: 'uppercase', alignSelf: 'flex-start' },
