...
import type { Student, SchoolMonth } from "@/types/student";
...
export function StudentCard({
  ...
  const statusColor =
    STATUS_COLOR[student.enrollment_status] ?? palette.zinc500;
        ...
        >
          <View>
            ...
            <View style={styles.walletRow}>
              ...
              <MonoText size="sm" color={palette.green500}>
            ...
            {student.student_type === "subscription" &&
              (student.monthly_payments ?? []).length > 0 && (
                <View style={styles.monthsRow}>
                  {(SCHOOL_MONTHS as readonly SchoolMonth[]).map((month) => {
                    const payment = student.monthly_payments.find(
                      (p) => p.school_month === month,
                    );
