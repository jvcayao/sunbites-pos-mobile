...
export function useStudentList(params: StudentListParams) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) =>
      studentsApi.list({ ...params, page: pageParam as number })
...
export function useUpdateType(id: number) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (student_type: string) => studentsApi.updateType(id, student_type),
    onSuccess: () => invalidateStudent(qc, id),
  })
}
...
export function useUpdatePaymentAmount(id: number) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ paymentId, amount }: { paymentId: number; amount: number }) =>
...
export function useAddSubscriptionRange(id: number) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (data: SubscriptionRangeDto) => studentsApi.addSubscriptionRange(id, data),
    onSuccess: () => qc.invalidateQueries({ queryKey: KEY.payments(id) }),
  })
}
