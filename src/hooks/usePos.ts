...
export function usePosTransactions(params: Omit<TransactionParams, 'page'>) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) =>
      posApi.transactions({ ...params, page: pageParam as number })
...
export function useUpdateMenuItem() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: Parameters<typeof posApi.updateMenuItem>[1] }) =>
...
export function useAdjustStock() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: Parameters<typeof posApi.adjustStock>[1] }) =>
