...
const mockLogs = referencesApi.inventory.logs as jest.MockedFunction<
  typeof referencesApi.inventory.logs
>;
...
describe("useInventoryItemLogs", () => {
  it("queries /references/inventory/{id}/logs and returns data", async () => {
    const logs = [
      { id: 1, type: "restock", quantity_change: 10, stock_after: 20 },
    ];
    mockLogs.mockResolvedValue({ data: logs } as any);

    const { result } = renderHook(() => useInventoryItemLogs(5), {
  ...
  it("uses query key [references, inventory-item-logs, id]", async () => {
    ...
    const { result } = renderHook(() => useInventoryItemLogs(99), {
      wrapper: createWrapper(),
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
