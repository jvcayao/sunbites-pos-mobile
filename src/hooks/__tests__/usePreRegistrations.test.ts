...
describe('usePreRegistrationDetail', () => {
  it('returns detail from API', async () => {
    ...
    const { result } = renderHook(() => usePreRegistrationDetail(5), { wrapper: createWrapper() })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
  ...
  it('transitions to error state on API failure', async () => {
    ...
    const { result } = renderHook(() => usePreRegistrationDetail(99), { wrapper: createWrapper() })
...
describe('usePendingCount', () => {
  it('returns total from meta when fetching pending with per_page=1', async () => {
    mockApi.list.mockResolvedValue({
      data: {
        data: [],
        meta: { current_page: 1, last_page: 1, total: 7, per_page: 1, from: 0, to: 0 },
        links: { first: '', last: '', prev: null, next: null },
      },
    } as any)

    const { result } = renderHook(() => usePendingCount(), { wrapper: createWrapper() })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))

    expect(result.current.data).toBe(7)
    expect(mockApi.list).toHaveBeenCalledWith({ status: 'pending', per_page: 1 })
  })
  ...
  it('returns 0 when API returns total of 0', async () => {
    mockApi.list.mockResolvedValue({
      data: {
        data: [],
        meta: { current_page: 1, last_page: 1, total: 0, per_page: 1, from: 0, to: 0 },
        links: { first: '', last: '', prev: null, next: null },
      },
    } as any)
...
describe('useUpdatePreRegistration', () => {
  it('calls update API and invalidates detail on success', async () => {
    ...
    const { result } = renderHook(() => useUpdatePreRegistration(), { wrapper: createWrapper() })
...
describe('useApprovePreRegistration', () => {
  it('calls approve API and invalidates list, detail, and pending count on success', async () => {
    ...
    const { result } = renderHook(() => useApprovePreRegistration(), { wrapper: createWrapper() })
    const invalidateSpy = jest.spyOn(queryClient, 'invalidateQueries')
    ...
    expect(invalidateSpy).toHaveBeenCalledWith(
      expect.objectContaining({ queryKey: ['pre-registrations-pending-count'] }),
    )
  })
})
...
describe('useRejectPreRegistration', () => {
  it('calls reject API with reason and invalidates list, detail, pending count', async () => {
    ...
    const { result } = renderHook(() => useRejectPreRegistration(), { wrapper: createWrapper() })
    const invalidateSpy = jest.spyOn(queryClient, 'invalidateQueries')
    result.current.mutate({ id: 8, rejection_reason: 'Incomplete details provided here.' })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
