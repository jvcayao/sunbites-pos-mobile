...
const mockGetLinkedStock = posApi.getLinkedStock as jest.MockedFunction<
  typeof posApi.getLinkedStock
>;
const mockAttachLinkedStock = posApi.attachLinkedStock as jest.MockedFunction<
  typeof posApi.attachLinkedStock
>;
const mockDetachLinkedStock = posApi.detachLinkedStock as jest.MockedFunction<
  typeof posApi.detachLinkedStock
>;
...
describe("useLinkedStock", () => {
  it("queries /references/menu-items/{id}/ingredients", async () => {
    ...
    const { result } = renderHook(() => useLinkedStock(10), {
...
describe("useAttachLinkedStock", () => {
  it("calls posApi.attachLinkedStock and invalidates pos-linked-stock + pos-menu-items", async () => {
    ...
    const { result } = renderHook(() => useAttachLinkedStock(), {
      wrapper: createWrapper(),
    });

    result.current.mutate({
      menuItemId: 10,
      data: { inventory_item_id: 3, quantity_used: 1 },
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(mockAttachLinkedStock).toHaveBeenCalledWith(10, {
      inventory_item_id: 3,
      quantity_used: 1,
    });
  });
...
describe("useDetachLinkedStock", () => {
  it("calls posApi.detachLinkedStock and invalidates pos-linked-stock + pos-menu-items", async () => {
    ...
    const { result } = renderHook(() => useDetachLinkedStock(), {
      wrapper: createWrapper(),
    });
