...
describe('useReminderBellCount', () => {
  it('returns bell count from API', async () => {
    ...
    const { result } = renderHook(() => useReminderBellCount(), { wrapper: createWrapper() })
...
describe('useEligibleParents', () => {
  it('fetches first page via useInfiniteQuery', async () => {
    ...
    const { result } = renderHook(() => useEligibleParents(), { wrapper: createWrapper() })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
...
describe('useSendReminders', () => {
  it('calls send API and invalidates bell-count and eligible-parents on success', async () => {
    mockApi.send.mockResolvedValue({ data: { sent: 2, skipped: 0, skipped_names: [] } } as any)
  ...
  it('passes force flag to API', async () => {
    mockApi.send.mockResolvedValue({ data: { sent: 1, skipped: 0, skipped_names: [] } } as any)
...
describe('useReminderParentDetail', () => {
  it('returns parent detail from API', async () => {
    const detail = { id: 1, full_name: 'Maria Santos', email: 'maria@example.com', phone: null, students: [] }
    mockApi.parentDetail.mockResolvedValue({ data: detail } as any)

    const { result } = renderHook(() => useReminderParentDetail(1), { wrapper: createWrapper() })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
  ...
  it('transitions to error state on API failure', async () => {
    ...
    const { result } = renderHook(() => useReminderParentDetail(99), { wrapper: createWrapper() })
    await waitFor(() => expect(result.current.isError).toBe(true))
  })
})
