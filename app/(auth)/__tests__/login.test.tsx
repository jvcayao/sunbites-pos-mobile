...
describe('LoginScreen — successful login', () => {
  it('calls authApi.login with the entered credentials', async () => {
    mockAuthApi.login.mockResolvedValue({ data: { token: 'tok', user: { id: 1 } } } as any)
    mockLogin.mockResolvedValue(undefined)
    renderLogin()
    fireEvent.changeText(screen.getByTestId('email-input'), 'user@sunbites.com')
    fireEvent.changeText(screen.getByTestId('password-input'), 'password123')
    fireEvent.press(screen.getByTestId('login-submit'))
    await waitFor(() => {
      expect(mockAuthApi.login).toHaveBeenCalledWith('user@sunbites.com', 'password123')
    })
  })
})
...
describe('LoginScreen — error states', () => {
  it('shows generic auth error for 401 response (prevents email enumeration)', async () => {
    const err = new axios.AxiosError('Unauthorized', '401', {} as any, {}, {
      status: 401,
  ...
  it('shows rate-limit message for 429 response', async () => {
    const err = new axios.AxiosError('Too Many Requests', '429', {} as any, {}, {
      status: 429,
      data: {},
    } as any)
    mockAuthApi.login.mockRejectedValue(err)
    ...
    fireEvent.press(screen.getByTestId('login-submit'))
    await waitFor(() => {
      expect(screen.getByText('Too many attempts. Please wait a minute.')).toBeTruthy()
    })
  })
...
describe('LoginScreen — loading state', () => {
  it('disables the submit button while request is in flight', async () => {
    ...
    act(() => { resolve({ data: { token: 'tok', user: { id: 1 } } }) })
  })
})
