...
import {
  useMealPlanner,
  useUpdateMealPlanner,
  useResetMealPlanner,
  useUpdateMealPlannerVisibility,
...
const DAYS: MealDay["day_of_week"][] = [
  "monday",
  "tuesday",
  "wednesday",
  "thursday",
...
const COLS: (keyof Omit<MealDay, "day_of_week">)[] = [
  "ulam",
  "vegetables",
  "fruit",
  "soup",
...
export default function MealPlannerScreen() {
  ...
  const updateCell = (day: string, col: string, value: string): void => {
    setLocalDays((prev) =>
      prev.map((d) => (d.day_of_week === day ? { ...d, [col]: value } : d)),
    );
  };

  const handleSave = (): void => {
    save(
      { month, week_number: week, days: localDays },
      {
        onSuccess: () => {
    ...
    toggle(
      {
        month,
        week_number: week,
        visible_to_parents: !plan?.visible_to_parents,
      },
      {
        onSuccess: () => {
  ...
  return (
    <View style={styles.container}>
          ...
          <>
            {plan !== undefined && (
              <Chip
                compact
                style={
                  plan.visible_to_parents
                    ? styles.visibleChip
                    : styles.hiddenChip
                }
                textStyle={{
                  fontSize: 11,
                  color: plan.visible_to_parents
                    ? palette.green500
                    : palette.zinc500,
                }}
                ...
                <Surface key={day} style={styles.gridRow} elevation={0}>
                  ...
                  {COLS.map((col, ci) => (
                    <View
                      ...
                      style={[
                        styles.mealCell,
                        { backgroundColor: COL_CELL_BG[ci] },
                      ]}
                    >
                      {editMode ? (
                        <TextInput
      ...
      <ConfirmDialog
        visible={showReset}
        title="Reset Week"
        message="Reset this week to default pattern?"
        confirmLabel="Reset"
        loading={isResetting}
      ...
      <ConfirmDialog
        ...
        confirmLabel={
          plan?.visible_to_parents ? "Yes, Hide It" : "Yes, Publish It"
        }
        confirmColor={
          plan?.visible_to_parents ? palette.red500 : palette.orange500
        }
        loading={isToggling}
