...
export default function SubscriptionConfigScreen() {
  ...
  const [dailyRate] = useState(0);

  const { data: amountsData } = useMonthlyAmounts(year);
  const { data: branchesData } = useBranchList();

  const amounts = Array.isArray(amountsData)
    ? amountsData
    : ((amountsData as any)?.data ?? []);
  ...
  const getCell = (branchId: number, month: SchoolMonth) =>
    amounts.find(
      (a: any) => a.branch_id === branchId && a.school_month === month,
    );
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      const payload = {
        branch_id: editCell.branchId,
        school_month: editCell.month,
        year,
        school_days: Number(days),
        amount: amount ? Number(amount) : undefined,
      };
      if (editCell.existing) {
        await referencesApi.subscriptionConfig.updateAmount(
          editCell.existing.id,
          payload,
        );
      } else {
        ...
        <View>
          ...
          {(SCHOOL_MONTHS as readonly SchoolMonth[]).map((month) => (
            <View key={month} style={styles.dataRow}>
              <View style={styles.monthCell}>
                <Text variant="bodySmall" style={styles.monthText}>
              ...
              {branches.map((b: any) => {
                ...
                return (
                  ...
                  >
                    <Text
                      variant="bodySmall"
                      style={cell ? styles.configured : styles.unconfigured}
                    >
                      {cell
                        ? `${cell.school_days}d · ${formatCurrency(cell.amount ?? 0)}`
                        : "—"}
                    </Text>
                  </Button>
                );
          ...
          >
            <Surface style={styles.modalSurface} elevation={4}>
              <Text variant="titleMedium" style={styles.modalTitle}>
                {editCell.month.toUpperCase()} ·{" "}
                {branches.find((b: any) => b.id === editCell.branchId)?.name}
              </Text>
              <Divider />
              ...
              <View style={styles.modalActions}>
                ...
                <Button mode="contained" onPress={handleSave} disabled={!days}>
                  Save
                </Button>
              </View>
            </Surface>
