...
function StatusBadgeInline({ status }: { status: string }) {
  ...
  if (s === "OK")
    return (
      ...
      >
        <Text style={[styles.badgeText, { color: "#16A34A" }]}>OK</Text>
      </View>
    );
  if (s === "LOW")
    return (
      ...
      >
        <Text style={[styles.badgeText, { color: "#CA8A04" }]}>LOW</Text>
      </View>
    );
  if (s === "OUT")
    return (
      ...
      >
        <Text style={[styles.badgeText, { color: "#DC2626" }]}>OUT</Text>
      </View>
    );
  if (s === "OVER")
    return (
      <View
        style={[
          styles.badge,
          { backgroundColor: OVER_BADGE.bg, borderColor: OVER_BADGE.border },
        ]}
      >
        <Text style={[styles.badgeText, { color: OVER_BADGE.text }]}>OVER</Text>
      </View>
    );
  return null;
}
...
export default function InventoryScreen() {
  ...
  const { data: itemLogs, isLoading: isLogsLoading } = useInventoryItemLogs(
    historyItem?.id ?? 0,
  );
  ...
  const { mutate: deleteItem, isPending: isDeleting } =
    useDeleteInventoryItem();
  const { mutate: archiveItem, isPending: isArchiving } =
    useArchiveInventoryItem();
  ...
  const historyList: InventoryLog[] = Array.isArray(history)
    ? history
    : ((history as any)?.data ?? []);
  const logs: InventoryLog[] = Array.isArray(itemLogs)
    ? itemLogs
    : ((itemLogs as any)?.data ?? []);

  const isFormValid =
    form.name.trim().length > 0 &&
    form.unit.trim().length > 0 &&
    form.restock_threshold.trim().length > 0;
  ...
  const handleCreate = (): void => {
    const payload = {
      ...
      ...(form.overstock_threshold
        ? { overstock_threshold: Number(form.overstock_threshold) }
        : {}),
      ...(form.cost_per_unit
          ...
          <Surface style={styles.addForm} elevation={0}>
            ...
            <View style={styles.formRow}>
              ...
              <TextInput
                ...
                onChangeText={(v) =>
                  setForm((f) => ({ ...f, restock_threshold: v }))
                }
                style={styles.inputFlex}
                dense
                keyboardType="numeric"
                accessibilityLabel="Low alert quantity"
              />
            </View>
            <View style={styles.formRow}>
              <TextInput
                ...
                onChangeText={(v) =>
                  setForm((f) => ({ ...f, overstock_threshold: v }))
                }
                style={styles.inputFlex}
                dense
                keyboardType="numeric"
                accessibilityLabel="Overstock quantity"
              />
              <TextInput
                mode="outlined"
                label="Cost/Unit"
              ...
              ListFooterComponent={
                archived.length > 0 ? (
                  <View style={styles.archivedSection}>
                    <Text variant="labelMedium" style={styles.archivedLabel}>
        ...
        <FlatList
          ...
          renderItem={({ item }: { item: InventoryLog }) => (
            <View
              style={[
                styles.row,
                { backgroundColor: LOG_ROW_COLOR[item.type] ?? palette.white },
              ]}
            >
              <View style={styles.left}>
                <Text variant="bodyMedium" style={styles.name}>
                ...
                <Text variant="bodySmall" style={styles.meta}>
                  {item.adjusted_by_name} · {item.reason}
                </Text>
              </View>
              <View style={styles.right}>
                  ...
                  {logs.map((log) => (
                    <View
                      ...
                      style={[
                        ...
                        {
                          backgroundColor:
                            LOG_ROW_COLOR[log.type] ?? palette.white,
                        },
                      ]}
                    >
                      <Text variant="bodySmall" style={styles.meta}>
                      ...
                      {log.reason ? (
                        <Text variant="bodySmall" style={styles.meta}>
                          {log.reason} · {log.adjusted_by_name}
                        </Text>
                      ) : null}
                    </View>
                  ))}
