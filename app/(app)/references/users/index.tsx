...
export default function UsersScreen() {
      ...
      >
        <View style={[listCardStyle, viewStyles.row]}>
          ...
          <View style={viewStyles.info}>
            <Text variant="bodyMedium" style={textStyles.name}>
              {item.full_name}
            </Text>
            <View style={viewStyles.badges}>
              {(item.roles ?? []).map((r: UserRole) => (
  ...
  return (
    <View style={viewStyles.container}>
      ...
      />
      <FilterChipRow>
        {(["all", "admin", "manager", "supervisor", "cashier"] as const).map(
          (r) => (
            <FilterChip
              key={r}
              label={r}
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListFooterComponent={
            isFetchingNextPage ? (
              <Text style={textStyles.meta}>Loading…</Text>
            ) : null
          }
        />
      )}
