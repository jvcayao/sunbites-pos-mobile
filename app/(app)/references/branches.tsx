...
export default function BranchesScreen() {
  ...
  useEffect(() => {
    if (!isAdmin) router.replace("/(app)/references");
  }, [isAdmin]);
    ...
    ({ item }: { item: any }) => (
      <Surface style={[listCardStyle, styles.card]} elevation={1}>
        <View style={styles.cardHeader}>
          ...
          <Chip
            compact
            style={item.is_active ? styles.activeChip : styles.inactiveChip}
            textStyle={{
              fontSize: 11,
              color: item.is_active ? palette.green500 : palette.zinc500,
            }}
          >
            {item.is_active ? "Active" : "Inactive"}
          </Chip>
        </View>
        {item.gcash_number !== null && (
          <Text variant="bodySmall" style={styles.meta}>
        ...
        <View style={styles.cardActions}>
          ...
          <Button
            compact
            mode="text"
            textColor={item.is_active ? palette.red500 : palette.green500}
