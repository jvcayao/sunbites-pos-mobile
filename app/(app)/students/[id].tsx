...
export default function StudentDetailScreen() {
  ...
  const studentId = parsed.success ? parsed.data : 0;
  ...
  const [, setEditContact] = useState<StudentContact | null>(null);
  const [deleteContact, setDeleteContact] = useState<StudentContact | null>(
    null,
  );

  const { data: student, isLoading, refetch } = useStudentDetail(studentId);
  const { data: contactsData, refetch: refetchContacts } =
    useStudentContacts(studentId);
  ...
  const { mutate: updateStatus } = useUpdateStatus(studentId);
  const { mutate: regenerateQr, isPending: isRegen } =
    useRegenerateQr(studentId);
  const { mutate: togglePayment } = useTogglePayment(studentId);
  const { mutate: updatePayAmt, isPending: isUpdatingAmt } =
    useUpdatePaymentAmount(studentId);
  const { mutate: addPeriod, isPending: isAddingPeriod } =
    useAddSubscriptionRange(studentId);
  const { mutate: removeContact, isPending: isRemovingContact } =
    useRemoveContact(studentId);
  const { mutate: resendActivation } = useResendActivation(studentId);

  const orders = ordersData?.pages.flatMap((p: any) => p.data ?? []) ?? [];
  const walletTx = Array.isArray(walletData)
    ? walletData
    : ((walletData as any)?.data ?? []);
  const contacts = Array.isArray(contactsData)
    ? (contactsData as StudentContact[])
  ...
  const handleStatusChange = (
    ...
    updateStatus(
      { enrollment_status: status, reason },
      {
        onSuccess: () => {
  ...
  const renderOrder = useCallback(
    ({ item }: { item: Order }) => (
      <View style={styles.orderRow}>
        <Text variant="labelSmall" style={styles.orderReceipt}>
          {item.receipt_number}
        </Text>
        ...
        <View style={styles.orderRight}>
          ...
          <Text variant="bodySmall" style={styles.meta}>
            {formatDate(item.created_at, "MMM d")}
          </Text>
        </View>
      </View>
    ),
    ...
    <View style={styles.container}>
          ...
          >
            ...
            {canDeleteStudent && (
              <Menu.Item
                ...
                titleStyle={{ color: palette.red500 }}
              />
            )}
          </Menu>
        }
      ...
      <Surface style={styles.statsBar} elevation={1}>
        <View style={styles.statItem}>
          ...
          <Text variant="titleSmall" style={styles.statGreen}>
        ...
        {student.credit_balance > 0 && (
          <View style={styles.statItem}>
            ...
            <Text variant="titleSmall" style={styles.statRed}>
              {formatCurrency(student.credit_balance)}
            </Text>
          </View>
        )}
        <View style={styles.statItem}>
            ...
            <DataTable>
              {[
                ...
                <DataTable.Row key={label}>
                  <DataTable.Cell textStyle={styles.tableKey}>
                    {label}
                  </DataTable.Cell>
            ...
            renderItem={({ item }: { item: any }) => (
              <View style={styles.txRow}>
                <View style={styles.txLeft}>
                  <Text variant="bodySmall" style={styles.txType}>
                    {item.type}
                  </Text>
                  {item.note !== undefined && (
                    <Text variant="bodySmall" style={styles.meta}>
                  ...
                  <Text variant="bodySmall" style={styles.meta}>
                    {formatDate(item.created_at, "MMM d, h:mm a")}
                  </Text>
                </View>
                <Text
                  variant="titleSmall"
                  style={item.amount >= 0 ? styles.statGreen : styles.statRed}
                >
                  {item.amount >= 0 ? "+" : ""}
      ...
      {tab === "logs" && (
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState icon="history" title="No activity logs" />
          }
        />
      )}
      ...
      {showPrintQr && (
        <PrintQrSheet
          ...
          onDismiss={() => setShowPrintQr(false)}
        />
      )}
      <ConfirmDialog
      ...
      {editPayment !== null && (
        <EditPaymentAmountSheet
          ...
          onConfirm={(amount) => {
            updatePayAmt(
              { paymentId: editPayment.id, amount },
              {
                onSuccess: () => {
