...
export default function InventoryReportScreen() {
  ...
  const { data, isLoading, refetch, isRefetching } = useInventoryReport({ status: statusF === 'all' ? undefined : statusF })
        ...
        <>
          {summary !== undefined && (
            <View style={styles.summaryRow}>
              <SummaryCard label="Out of Stock"     value={summary.out_of_stock}    accent={palette.red500} />
              <SummaryCard label="Below Threshold"  value={summary.below_threshold} accent={palette.yellow500} />
              <SummaryCard label="Overstock"        value={summary.overstock}       accent={palette.blue500} />
            </View>
          )}
          <SegmentedButtons
            value={view}
            onValueChange={(v) => setView(v as any)}
            buttons={[{ value: 'snapshot', label: 'Snapshot' }, { value: 'history', label: 'History' }]}
            style={styles.tabs}
          />
          {view === 'snapshot' && (
            <>
              <FilterChipRow>
                {(['all', 'ok', 'low', 'out', 'over'] as const).map((s) => (
                  <FilterChip key={s} label={s.toUpperCase()} active={statusF === s} onPress={() => setStatusF(s)} />
                ))}
              </FilterChipRow>
              <FlatList data={snapshot} keyExtractor={(i: InventoryReportItem) => String(i.id)} renderItem={renderSnapshot} refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} tintColor={palette.orange500} />} ListEmptyComponent={<EmptyState icon="package-variant-closed" title="No inventory data" />} />
            </>
          )}
          {view === 'history' && (
            <FlatList data={history} keyExtractor={(i: InventoryLogItem) => String(i.id)} renderItem={renderLog} ListEmptyComponent={<EmptyState icon="history" title="No history" />} />
          )}
        </>
      )}
    </View>
