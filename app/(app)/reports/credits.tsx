...
import {
  DatePresetPicker,
  type DateRange,
} from "@/components/shared/DatePresetPicker";
...
export default function CreditsReportScreen() {
      ...
      <View style={[listCardStyle, styles.row]}>
        <View style={styles.left}>
          ...
          <Text variant="bodySmall" style={styles.meta}>
            {item.grade_level} · {formatDate(item.created_at, "MMM d")}
          </Text>
        </View>
        <View style={styles.right}>
          ...
          <Text
            variant="titleSmall"
            style={item.type === "settled" ? styles.settled : styles.charged}
          >
  ...
  return (
    <View style={styles.container}>
      <AppHeader title="Credits Report" showBack />
      {isLoading ? (
        ...
        <>
          {summary !== undefined && (
            <View style={styles.summaryRow}>
              ...
              <SummaryCard
                label="Settled"
                value={formatCurrency(summary.total_settled)}
                accent={palette.green500}
              />
              <SummaryCard
                label="Outstanding"
                value={formatCurrency(summary.net_outstanding)}
          ...
          <FilterChipRow>
            ...
            />
            {(["all", "charged", "settled", "voided"] as const).map((t) => (
              <FilterChip
                key={t}
                label={t}
                active={type === t}
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <EmptyState icon="alert-circle" title="No credit records" />
            }
            ListFooterComponent={
              isFetchingNextPage ? (
