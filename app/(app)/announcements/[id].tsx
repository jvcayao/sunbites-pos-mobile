...
export default function AnnouncementDetailScreen(): React.JSX.Element {
  ...
  const { data, isLoading, refetch, isRefetching } = useAnnouncementDetail(numericId)
  ...
  const badge = data !== undefined ? BADGE_COLORS[data.recipient_type] : undefined
        ...
        <FlatList
          ...
          ListHeaderComponent={() => (
            <View style={styles.header}>
              ...
              {data.title !== null && (
                <Text variant="titleMedium" style={styles.title}>{data.title}</Text>
              )}

              <Text variant="bodyMedium" style={styles.message}>{data.message}</Text>
...
const styles = StyleSheet.create({
  ...
  readSummary:     { backgroundColor: palette.zinc100, borderRadius: 8, padding: 12, marginTop: 4 },
