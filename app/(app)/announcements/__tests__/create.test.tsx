...
const mockUseCreateAnnouncement = useCreateAnnouncement as jest.MockedFunction<typeof useCreateAnnouncement>
...
describe('CreateAnnouncementScreen — initial render', () => {
  it('shows message input and recipient type pill', async () => {
    mockUseCreateAnnouncement.mockReturnValue({ mutate: jest.fn(), isPending: false, isSuccess: false } as any)
...
describe('CreateAnnouncementScreen — validation', () => {
  it('shows error when trying to send with no recipients selected', async () => {
    ...
    mockUseCreateAnnouncement.mockReturnValue({ mutate: mockMutate, isPending: false, isSuccess: false } as any)
    render(<CreateAnnouncementScreen />, { wrapper: Wrapper })
    await waitFor(() => expect(screen.getByTestId('message-input')).toBeTruthy())
    ...
    await waitFor(() => {
      expect(screen.getByText('Please select at least one recipient')).toBeTruthy()
    })
...
describe('CreateAnnouncementScreen — submit', () => {
  it('calls create mutation with correct payload when form is valid', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('message-input')).toBeTruthy())
    fireEvent.changeText(screen.getByTestId('message-input'), 'Important announcement')

    await waitFor(() => expect(screen.getByTestId('recipient-row-1')).toBeTruthy())
  ...
  it('shows success toast and navigates back on successful submission', async () => {
    ...
    mockUseCreateAnnouncement.mockReturnValue({ mutate, isPending: false, isSuccess: false } as any)
    render(<CreateAnnouncementScreen />, { wrapper: Wrapper })
