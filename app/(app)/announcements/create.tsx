...
export default function CreateAnnouncementScreen(): React.JSX.Element {
  ...
  const { data: parentsData, isLoading: isLoadingParents } = useQuery<RecipientOption[]>({
  ...
  const { data: staffData, isLoading: isLoadingStaff } = useQuery<RecipientOption[]>({
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
        <View style={styles.field}>
          <Text variant="labelMedium" style={styles.label}>Title (optional)</Text>
          <TextInput
            testID="title-input"
            value={title}
        ...
        <View style={styles.field}>
          <View style={styles.labelRow}>
            <Text variant="labelMedium" style={styles.label}>Message *</Text>
            <Text variant="bodySmall" style={styles.charCount}>{message.length}/1000</Text>
          </View>
          <TextInput
        ...
        <View style={styles.field}>
          ...
          <RecipientTypePill value={recipientType} onChange={handleTypeChange} />
        </View>

        <View style={styles.field}>
          <Text variant="labelMedium" style={styles.label}>Recipients</Text>
          {recipientError !== null && (
            <Text variant="bodySmall" style={styles.errorText}>{recipientError}</Text>
          )}
          <RecipientSelector
...
const styles = StyleSheet.create({
  ...
  input:      { borderWidth: 1, borderColor: palette.zinc200, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8, color: palette.zinc900, fontSize: 14, backgroundColor: palette.white },
  textarea:   { minHeight: 96, textAlignVertical: 'top' },
  errorText:  { color: palette.red500 },
  overlay:    { ...StyleSheet.absoluteFill, backgroundColor: 'rgba(255,255,255,0.7)', alignItems: 'center', justifyContent: 'center' },
})
