...
export default function PosScreen() {
  ...
  const {
    items: cartItems, student, isWalkIn,
    addItem, setStudent, setIsWalkIn, clearCart,
  } = useCartStore()
  ...
  // Transaction filters
  const [txDateRange, setTxDateRange] = useState<DateRange>({ from: '', to: '' })
  ...
  const { data: txPages, refetch: refetchTx, isFetchingNextPage, fetchNextPage, hasNextPage, isRefetching: isRefetchingTx } = usePosTransactions(txParams)
  ...
  const handleSelectStudent = useCallback((s: PosStudent) => {
    setStudent(s)
    setIsWalkIn(false)
  ...
  const handleVoidOrder = (reason: string): void => {
    ...
    voidOrder({ id: voidTarget.id, reason }, {
      onSuccess: () => {
    ...
    <View style={styles.container}>
      ...
      {tab === 'pos' && (
        showSplitPane ? (
          ...
          <View style={styles.phonePos}>
            ...
            <FAB
              icon="cart"
              label={cartItems.length > 0 ? `View Cart (${cartItems.length}) · ${formatCurrency(cartItems.reduce((s, i) => s + i.menuItem.price * i.quantity, 0))}` : 'Cart (empty)'}
            ...
            <Modal visible={showCartSheet} animationType="slide">
              <View style={styles.cartModal}>
                ...
                <CartPanel onCheckout={(p) => { handleCheckout(p); setShowCartSheet(false) }} loading={isCheckingOut} />
              </View>
            </Modal>
          </View>
        )
      ...
      {tab === 'transactions' && (
        <View style={styles.tabContent}>
          <FilterChipRow>
            <FilterChip label="Date" active={!!txDateRange.from} onPress={() => setShowDatePicker(true)} />
            {(['all', 'cash', 'gcash', 'wallet', 'subscription'] as const).map((m) => (
              <FilterChip key={m} label={m === 'all' ? 'All Methods' : m} active={txPayment === m} onPress={() => setTxPayment(m)} />
            ))}
          </FilterChipRow>
          <FilterChipRow>
            {(['all', 'completed', 'voided'] as const).map((s) => (
              <FilterChip key={s} label={s === 'all' ? 'All Status' : s} active={txStatus === s} onPress={() => setTxStatus(s)} />
            ))}
          </FilterChipRow>
          {txOrders.length === 0 ? (
            ...
            <FlatList
              ...
              refreshControl={<RefreshControl refreshing={isRefetchingTx} onRefresh={refetchTx} tintColor={palette.orange500} />}
              onEndReached={() => { if (hasNextPage) void fetchNextPage() }}
              onEndReachedThreshold={0.2}
              ListFooterComponent={isFetchingNextPage ? <Text style={styles.loadingMore}>Loading…</Text> : null}
            />
          )}
          <DatePresetPicker
            visible={showDatePicker}
            value={txDateRange}
            onConfirm={(range) => { setTxDateRange(range); setShowDatePicker(false) }}
...
const styles = StyleSheet.create({
  ...
  splitLeft: { flex: 3 },
  splitRight: { flex: 2, borderLeftWidth: StyleSheet.hairlineWidth, borderLeftColor: palette.zinc200 },
  cartFab: {
    position: 'absolute',
    bottom: 24,
    right: 16,
