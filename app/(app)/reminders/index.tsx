...
export default function RemindersScreen(): React.JSX.Element {
  ...
  const unsentParents = parents.filter((p) => p.unpaid_periods.some((period) => !period.was_sent))
  ...
  const alreadySentSelectedParents = parents
    .filter((p) => selected.has(p.id) && p.unpaid_periods.every((period) => period.was_sent))
    .map((p) => ({
      id: p.id,
      full_name: p.full_name,
      last_sent_at: p.unpaid_periods.find((period) => period.last_sent_at)?.last_sent_at ?? '',
    }))
  ...
  const executeSend = (force: boolean): void => {
    ...
    sendReminders(
      ...
      {
        onSuccess: (result) => {
          toast.success(`Sent to ${result.sent} parent${result.sent !== 1 ? 's' : ''}`)
          if (result.skipped > 0) {
            toast.error(
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onEndReached={() => hasNextPage && !isFetchingNextPage && void fetchNextPage()}
